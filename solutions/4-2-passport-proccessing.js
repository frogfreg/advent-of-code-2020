...
const validateValues = (string) => {
  const passportData = parsePassport(string);

  if (parseInt(passportData.byr) < 1920 || parseInt(passportData.byr) > 2002) {
    console.log("byr");
    return false;
  }
  if (parseInt(passportData.iyr) < 2010 || parseInt(passportData.iyr) > 2020) {
    console.log("iyr");

    return false;
  }
  if (parseInt(passportData.eyr) < 2020 || parseInt(passportData.iyr) > 2030) {
    console.log("eyr");

    return false;
  }
  if (passportData.pid.length !== 9) {
    console.log("pid");
    return false;
  } else if (Number.isNaN(parseInt(passportData.pid))) {
    console.log("pid NaN");
    return false;
  }
  if (
  ...
  if (!passportData.hcl.match(/^#([a-f0-9]{6})$/g)) {
    console.log("hcl");
    return false;
  }
  if (!(passportData.hgt.includes("cm") || passportData.hgt.includes("in"))) {
    console.log("hgt");
    return false;
  } else if (passportData.hgt.includes("cm")) {
    // const cm = parseInt(passportData.hgt.replace("cm", ""));
    const cm = passportData.hgt.slice(0, passportData.hgt.length - 2);
    if (Number.isNaN(parseInt(cm)) || cm < 150 || cm > 193) {
      return false;
    }
  } else if (passportData.hgt.includes("in")) {
    // const inches = parseInt(passportData.hgt.replace("in", ""));
    const inches = passportData.hgt.slice(0, passportData.hgt.length - 2);
    if (Number.isNaN(parseInt(inches)) || inches < 59 || inches > 76) {
      return false;
    }
  }
...
// console.log(
//   validateValues(
//     "pid:0123456789 hgt:175cm ecl:brn iyr:2012 eyr:2030 byr:2002 hcl:#123abc"
//   )
// );
const rawArr = `eyr:2029 byr:1931 hcl:z cid:128
...
pid:394421140
ecl:gry
cid:209 hcl:#efcc98 byr:2001`.split("\n\n");

const cleanArr = inputArr.map((string) => {
  return string.replace(/\n/g, " ");
});

const result = cleanArr.reduce((acc, cur) => {
  if (validateKeys(cur)) {
    if (validateValues(cur)) {
      return acc + 1;
    } else {
      return acc;
    }
